...
def extract_bindings(model_output: str, binding_schema: dict) -> dict:
    ...
    ----------
    model_output : str
        The raw text produced by the AI model.
    binding_schema : dict
        A mapping from variable names to extraction rules.
        Each rule is a dict with a 'method' key and method‑specific fields.

        Supported methods:
          - "regex"   : {"method": "regex", "pattern": "<regex with named group 'value'>"}
          - "jsonpath": {"method": "jsonpath", "path": "<simple dotted path>"}
          - "tag"     : {"method": "tag", "tag": "TAG_NAME"}
          - "constant": {"method": "constant", "value": <integer>}
    ...
    dict
        {
          ...
          "bindings": {"var1": 42, ...},
          "missing": ["var2", ...]   # only present when INCOMPLETE
        }
    """
    bindings = {}
    missing = []
...
def _extract_constant(rule: dict):
    ...
    if isinstance(val, bool):
        return None  # booleans are not valid integers for bindings
...
def _extract_jsonpath(model_output: str, rule: dict, var_name: str):
    ...
    # Try to parse the whole string first (pure JSON output).
    obj = None
    try:
        obj = json.loads(model_output)
    except (json.JSONDecodeError, TypeError):
        # If that fails, look for the first JSON object in the string.
...
def _parse_int(value):
    ...
    Booleans are rejected (Python bool is a subclass of int)."""
