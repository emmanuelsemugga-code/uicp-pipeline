#!/usr/bin/env python3
"""
phase5_engine.py – UICP Phase 5 Trust & Audit Engine (GAP‑42 patched)
Includes verify_decision_record() + built‑in 30‑test harness.
"""
import hashlib, json
...
# ----- GAP‑42 verification function -----
def verify_decision_record(decision: dict, gateway_public_key) -> tuple:
    """
    GAP‑42 PATCH: Verify that a Phase 4 decision record was produced
    by the legitimate gateway holding the corresponding private key.

    Returns (True, None) if verification passes.
    Returns (False, reason_string) if verification fails.
    Never raises an exception — all errors are returned as (False, reason).
    """
    try:
        ...
        try:
            ...
            return False, f"decision record missing required field: {e}"
